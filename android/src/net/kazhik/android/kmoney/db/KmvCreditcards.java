...
public class KmvCreditcards extends KmTable {
	private static final String VIEW_NAME = "kmv_creditcards";
	private static final String CREATE_VIEW =
			...
			+ "A.user_id,"
			+ "count(*) AS trns "
			+ "FROM " + KmCreditCardInfo.TABLE_NAME + " A "
			+ "LEFT JOIN " + KmCreditCardTrns.TABLE_NAME + " B "
			+ "ON A.id = B.card_id and A.user_id = B.user_id "
			+ "GROUP BY A.id";

	public KmvCreditcards(Context context) {
    	super(context);
	}
