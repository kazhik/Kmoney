...
public class KmvEMoneys extends KmTable {
	private static final String VIEW_NAME = "kmv_emoneys";
	private static final String CREATE_VIEW =
			...
			+ "A.user_id,"
			+ "count(*) AS trns "
			+ "FROM " + KmEMoneyInfo.TABLE_NAME + " A "
			+ "LEFT JOIN " + KmEMoneyTrns.TABLE_NAME + " B "
			+ "ON A.id = B.money_id and A.user_id = B.user_id "
			+ "GROUP BY A.id";

	public KmvEMoneys(Context context) {
    	super(context);
	}
